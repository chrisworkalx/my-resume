...
- 根据jira wiki BTS 分派的任务 参加指定项目的开发迭代任务。
- perm环境部署、charles抓包代理、上线发布系统、kibana、graylog定位生产日志问题。
- 使用handlebars、vue、react、nestjs、nextjs等框架开发任务
- 负责coupang网站及app端（火箭配送等）及h5页面的开发任务等
...
- Testing 项目基于vite + vue3 + element-ui-plus进行构建（参考vben开源）
...
**项目介绍：** coupang是一家专注做跨境电商，主要业务是线上销售各种商品，包括电子产品、美容产品、消费品、书籍、婴儿用品、家居用品、装饰、时尚、玩具、体育用品、门票以及旅游和文化活动策划等；还提供配送和退货时效性服务

**项目职责：**

- 主要针对BTS中分发的任务，参加prd评审，并针对jira中的文档进行阅读，并制定开发任务；排期等
- 因为需要跨服务器去访问资源，所以需要搭建本地的perm环境，并通过charles去代理抓包，构建本地的开发环境，因为页面开发采用的技术栈没有固定唯一并且前后端没有分离，开发构成中需要线上打包、包括原生AMD、vue、react等构建的单页面应用等
- h5部分也是基于混合开发，需要抓包代理、实施开发等
- app端（火箭配送） 前端开发的部分使用的技术栈是nestjs + nextjs + react模式开发等
- 需要根据测试用例跑完并通过、需要在日志系统管理像 graylog、kibana等监听jsLog功能性日志埋点等
- 发布系统 采用AB test 需要根据制定的版本号 进行释放开发等，部署前需要创建ticket，待相关负责人approve后、去发布系统释放锁，根据发布系统规则部署线上环境等
{style="page-break-after: always"}
...
- 对项目涉及的字典进行统一缓存调用处理等
- 配合后端 优化了大文件上传和下载
- 对于滚动列表中无限加载设计一些图片等进行长列表的优化和懒加载等
...
**技术栈：** vite + vue3 + echarts等

**项目介绍：** 这是一个主要侦测机械臂工作日志、安装包 commit 信息、公司部门团队任务完成度等展示系统

**项目职责：**

- 使用echarts开发一些展示统计的功能页面
- 涉及了dashboard页开发、jenkins、BitBucket等涉及版本控制集成页面等
{style="page-break-after: always"}
...
### 薪福易后台管理系统 + 薪福易(vue、h5) (壹账通)

**时间：** 2020.04-2020-08

**技术栈：** vue、element ui、js-cookie、vant、webpack

**项目介绍：** 针对服务数字化转型，实现企业人事管理以及代发薪资等功能的前台 H5 项目，内嵌了其他 几个系统。主要分为工资相关模块和广告模块
